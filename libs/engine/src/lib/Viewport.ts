export class Viewport {
  ...
  listeners: Array<(start: number, end: number) => void>;

  constructor(totalPoints: number, _initialZoom = 1.0) {
  ...
  zoom(factor: number, pivot = 0.5): void {
  ...
  onChange(callback: (start: number, end: number) => void): void {
  ...
  getRange(): { start: number; end: number; range: number } {
